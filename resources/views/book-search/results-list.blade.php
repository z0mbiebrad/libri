    ...
    @isset($books)
        ...
        @foreach ($books as $book)
            <article class="group grid mx-auto rounded-sm max-w-lg my-12 grid-cols-8 border border-neutral-300 bg-zinc-200 text-neutral-900 dark:border-neutral-700 dark:bg-zinc-800 dark:text-neutral-300 shadow-xl relative">
                ...
                <!-- body -->
                <div class="flex flex-col col-span-4 mx-6 sm:my-10 justify-center space-y-1">
                    ...
                    @endisset
                    @isset($book->epub)
                        <a class="bg-transparent rounded-radius py-2 text-base font-medium tracking-wide text-primary transition hover:opacity-75 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary active:opacity-100 active:outline-offset-0 disabled:opacity-75 disabled:cursor-not-allowed dark:text-primary-dark dark:focus-visible:outline-primary-dark flex items-center gap-x-1 hover:underline"
                            href="{{ $book->epub }}">
                            eBook {{ $book->price }}
